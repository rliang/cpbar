...
static void create_window(xcb_screen_t *s, int16_t x, int16_t y)
{
	xcb_create_window(window.xcb_connection, XCB_COPY_FROM_PARENT,
			window.window, s->root, x, y,
			window.window_width, window.window_height,
			0, XCB_WINDOW_CLASS_INPUT_OUTPUT,
			s->root_visual, XCB_CW_EVENT_MASK,
			(const uint32_t []){ [0] = XCB_EVENT_MASK_EXPOSURE });
	xcb_change_window_attributes(window.xcb_connection, window.window,
			XCB_CW_OVERRIDE_REDIRECT, (const uint32_t []){ 1 });
	xcb_map_window(window.xcb_connection, window.window);
}
...
void window_init(uint16_t height, bool on_bottom)
{
	...
	window.visualtype = get_root_visual_type(s);

	window.window = xcb_generate_id(window.xcb_connection);
	window.window_width = s->width_in_pixels;
...
{
	xcb_destroy_window(window.xcb_connection, window.window);
